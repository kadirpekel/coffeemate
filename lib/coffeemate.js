...
function Coffeemate (options) {
  ...
  Object.keys(connect.middleware).forEach(function (item) {
    self[item] = function () {
      ...
      var postprocess = args.length && typeof(args[0]) && args[0].postprocess && args.shift();
      var middleware = connect.middleware[item].apply(self, arguments);
      if (postprocess) {
        self.postprocessors.push(middleware);
      } else {
        self.use(middleware);
      }
      return middleware;
...
Coffeemate.prototype.use = function (route, handle, postprocess) {
  if (postprocess) {
    self.postprocessors.push({route: route, handle: handle});
  } else {
...
Coffeemate.prototype.listen = function () {
  ...
  self.postprocessors.forEach(function (middleware) {
    self.use(middleware);
  });
